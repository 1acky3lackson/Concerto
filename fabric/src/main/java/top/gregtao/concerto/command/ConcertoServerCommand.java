                                ...
                                CommandManager.literal("list").then(
                                        CommandManager.argument("page", IntegerArgumentType.integer(1)).executes(context -> {
                                            ConcertoRunner.run(() -> {
                                                ...
                                                Map<UUID, ServerMusicNetworkHandler.ServerMusicContext> map = ServerMusicNetworkHandler.WAIT_AUDITION;
                                                Iterator<Map.Entry<UUID, ServerMusicNetworkHandler.ServerMusicContext>> iterator = map.entrySet().iterator();
                                                ...
                                                for (int i = 10 * (page - 1); i < Math.min(10 * page, map.size()) && iterator.hasNext(); ++i) {
                                                    Map.Entry<UUID, ServerMusicNetworkHandler.ServerMusicContext> entry = iterator.next();
                                                    MusicDataPacket packet = entry.getValue().packet();
