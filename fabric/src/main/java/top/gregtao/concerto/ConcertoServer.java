...
import top.gregtao.concerto.network.FabricConcertoNetworking;
import top.gregtao.concerto.network.ServerMusicNetworkHandler;
import top.gregtao.concerto.platform.FabricPlatform;
...
public class ConcertoServer implements ModInitializer {
    ...
    public void onInitialize() {
        ConcertoCommon.init(new FabricPlatform());

        NeteaseCloudApiClient.QUALITY_SUPPLIER = () -> ClientConfig.INSTANCE.options.neteaseMusicQuality;
        KuGouMusicApiClient.LITE_SUPPLIER = () -> {
             if (FabricLoader.getInstance().getEnvironmentType() == EnvType.CLIENT) {
                 return ClientConfig.INSTANCE.options.kuGouMusicLite;
             } else {
                 return ServerConfig.INSTANCE.options.kuGouMusicLite;
             }
        };
        HttpRequestBuilder.PRINT_REQUEST_RESULTS_SUPPLIER = () -> ClientConfig.INSTANCE.options.printRequestResults;

        CommandRegistrationCallback.EVENT.register(ConcertoServerCommand::register);
        FabricConcertoNetworking.register();
