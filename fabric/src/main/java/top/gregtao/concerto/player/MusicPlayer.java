...
public class MusicPlayer extends StreamPlayer implements StreamPlayerListener {
    ...
    public void reloadConfig(Runnable callback) {
        ConcertoRunner.run(() -> {
            this.started = false;
            this.stop();
            MusicPlayerState state = MusicJsonParsers.fromRaw(ConcertoClient.MUSIC_CONFIG.read());
            if (state != null) {
                MusicPlayerHandler.INSTANCE = new MusicPlayerHandler(new java.util.ArrayList<>(state.getMusicList()), state.getCurrentIndex(), state.getOrderType());
            } else {
