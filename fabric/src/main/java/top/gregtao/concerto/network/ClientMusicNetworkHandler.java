...
public class ClientMusicNetworkHandler {
    ...
    public static void sendC2SMusicData(MusicDataPacket packet) {
        ...
        packet.music.load();
        ConcertoPayload buf = new ConcertoPayload(packet.toPacket(player.getName().getString()));
    ...
    public static void musicDataReceiver(ConcertoPayload payload, ClientPlayNetworking.Context context) {
        try {
            MusicDataPacket packet = MusicDataPacket.fromPacket(payload.toCommon(), true);
            PlayerEntity self = context.player();
            if (packet != null && packet.music != null && self != null) {
                addToWaitList(context.client(), packet, self);
            } else {
                ConcertoCommon.getPlatform().getLogger().warn("Received an unknown music data packet");
            }
        } catch (Exception e) {
            ConcertoCommon.getPlatform().getLogger().warn("Received an unsafe music data packet");
            // Ignore unsafe music
        }
    }
    ...
    public static void playerJoinHandshake(ConcertoPayload payload, ClientPlayNetworking.Context context) {
        ...
        if (args[1].equals("CallJoin")) {
            ...
            if (player != null && playerName.equals(player.getName().getString())) {
                ...
                ConcertoCommon.getPlatform().getLogger().info("Concerto has been installed in this server");
    ...
    public static void auditionDataSyncReceiver(ConcertoPayload payload, ClientPlayNetworking.Context context) {
        ...
        try {
            ...
            ConcertoCommon.getPlatform().getLogger().error("Received an AuditionSyncDataPacket with illegal UUID: {}", args[1]);
        }
    }
...
//          .peek(playlist -> MusicPlayerHandler.loadInThreadPool(playlist.getList())).toList(), () -> {
    ...
    public static void musicAgentJoin() {
        ClientPlayNetworking.send(new ConcertoPayload(CommonConcertoPayload.Channel.MUSIC_AGENT, "Join"));
        ConcertoClient.clientState = ConcertoClient.ClientState.MUSIC_AGENT;
    }

    public static void musicAgentQuit() {
        ClientPlayNetworking.send(new ConcertoPayload(CommonConcertoPayload.Channel.MUSIC_AGENT, "Quit"));
        ConcertoClient.clientState = ConcertoClient.ClientState.LOCAL;
    }

    public static void musicAgentNewVote() {
        ClientPlayNetworking.send(new ConcertoPayload(CommonConcertoPayload.Channel.MUSIC_AGENT, "Vote:New"));
    }

    public static void musicAgentQuery() {
        ClientPlayNetworking.send(new ConcertoPayload(CommonConcertoPayload.Channel.MUSIC_AGENT, "Query"));
    }

    public static void musicAgentVote(boolean vote) {
        ClientPlayNetworking.send(new ConcertoPayload(CommonConcertoPayload.Channel.MUSIC_AGENT, "Vote:" + (vote ? "1" : "0")));
    }
    ...
    public static boolean musicAgentAddMusic(Music music) {
        ...
        ClientPlayNetworking.send(new ConcertoPayload(CommonConcertoPayload.Channel.MUSIC_AGENT,
