...
public class ServerMusicNetworkHandler {
    ...
    public record ServerMusicContext(MusicDataPacket packet, MinecraftServer server) {}
    public static Map<UUID, ServerMusicContext> WAIT_AUDITION = new HashMap<>();

    public static void removeFirst() {
        Iterator<Map.Entry<UUID, ServerMusicContext>> iterator = WAIT_AUDITION.entrySet().iterator();
        if (!iterator.hasNext()) return;
        Map.Entry<UUID, ServerMusicContext> entry = iterator.next();
    ...
    public static void passAudition(@Nullable PlayerEntity auditor, UUID uuid) {
        if (WAIT_AUDITION.containsKey(uuid)) {
            ServerMusicContext context = WAIT_AUDITION.get(uuid);
            MusicDataPacket packet = context.packet;
            MinecraftServer server = context.server;
            WAIT_AUDITION.remove(uuid);
            boolean success = sendS2CMusicData(packet, server, true);
            if (auditor != null) {
                if (success) {
            ...
            sendS2CAuditionSyncData(uuid, context, true);
    ...
    public static void rejectAll(@Nullable PlayerEntity auditor) {
        WAIT_AUDITION.forEach((uuid, context) -> {
            MusicDataPacket packet = context.packet;
            MinecraftServer server = context.server;
            PlayerEntity player = server.getPlayerManager().getPlayer(packet.from);
    ...
    public static void sendAuditionSyncPacket(UUID uuid, ServerPlayerEntity player, MusicDataPacket packet, boolean isDelete) {
        ConcertoPayload payload = new ConcertoPayload(CommonConcertoPayload.Channel.AUDITION_SYNC, (isDelete ? "DEL;" : "ADD;") + uuid + ";" +
                (isDelete ? "QwQ" : Objects.requireNonNull(MusicJsonParsers.to(packet.music)).toString()));
        ServerPlayNetworking.send(player, payload);
    }

    public static void sendS2CAuditionSyncData(UUID uuid, ServerMusicContext context, boolean isDelete) {
        MusicDataPacket packet = context.packet;
        MinecraftServer server = context.server;
    ...
    public static void sendS2CAllAuditionData(ServerPlayerEntity player) {
        WAIT_AUDITION.forEach((uuid, context) -> sendAuditionSyncPacket(uuid, player, context.packet, false));
    }

    public static void sendS2CPresetRadiosPacket(ServerPlayerEntity player) {
        ConcertoPayload payload = new ConcertoPayload(CommonConcertoPayload.Channel.PRESET_RADIOS,
                PresetPlaylistsConfig.PRESET_RADIOS.toString());
        ServerPlayNetworking.send(player, payload);
    }

    public static boolean sendS2CMusicData(MusicDataPacket packet, MinecraftServer server, boolean audit) {
        if (!packet.isS2C) {
        ...
        ConcertoPayload payload = new ConcertoPayload(packet.toPacket());
        PlayerManager playerManager = server.getPlayerManager();
    ...
    public static void musicDataReceiver(ConcertoPayload payload, ServerPlayNetworking.Context context) {
        ...
        try {
            MusicDataPacket packet = MusicDataPacket.fromPacket(payload.toCommon(), false);
            if (packet != null && packet.music != null && server != null) {
                ...
                if (!playerExist(playerManager, packet.to)) {
                    ...
                    packet.isS2C = true;
                    // packet.server = server; // Handled by context now
                    ...
                    if (audit) {
                        ...
                        WAIT_AUDITION.put(uuid, new ServerMusicContext(packet, server));
                        if (WAIT_AUDITION.size() > ConcertoNetworking.WAIT_LIST_MAX_SIZE) {
                            removeFirst();
                        }
                    } else {
                        success = sendS2CMusicData(packet, server, false);
                    }
    ...
    public static void playerJoinHandshake(ServerPlayerEntity player) {
        ConcertoPayload payload = new ConcertoPayload(CommonConcertoPayload.Channel.HANDSHAKE,
    ...
    public static void musicAgentSendMusic(ServerPlayerEntity player, String music) {
        ConcertoPayload payload = new ConcertoPayload(CommonConcertoPayload.Channel.MUSIC_AGENT,
                TextUtil.toBase64(music));
        ServerPlayNetworking.send(player, payload);
    }

    public static void musicAgentSendStop(ServerPlayerEntity player) {
        ConcertoPayload payload = new ConcertoPayload(CommonConcertoPayload.Channel.MUSIC_AGENT, "Stop");
        ServerPlayNetworking.send(player, payload);
    }
