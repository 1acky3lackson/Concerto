...
import top.gregtao.concerto.player.MusicPlayerState;
...
public class MusicJsonParsers {
    ...
    public static Music from(JsonObject jsonObject, boolean withMeta) {
        try {
            ...
            ConcertoCommon.getPlatform().getLogger().warn("Error occurred when converting from JSON to music, {}: {}", e, jsonObject.toString());
            return null;
        }
    }
    ...
    public static JsonObject to(Music music, boolean withMeta) {
        try {
            ...
            ConcertoCommon.getPlatform().getLogger().warn("Error occurred when converting from music to JSON, {}: {}", e, music.getMeta());
            return null;
        }
    }
    ...
    public static Playlist fromPlaylist(JsonObject object, boolean withMeta) {
        try {
            ...
            ConcertoCommon.getPlatform().getLogger().warn("Error occurred when converting from JSON to playlist, {}: {}", e, object.toString());
            return null;
        }
    }
    ...
    public static JsonObject toPlaylist(Playlist playlist, boolean withMeta) {
        try {
            ...
            ConcertoCommon.getPlatform().getLogger().warn("Error occurred when converting from playlist to JSON, {}: {}", e, playlist.getMeta());
            return null;
        }
    }

    public static MusicPlayerState fromRaw(String json) {
        try {
            if (json.isEmpty()) return new MusicPlayerState();
            ...
            return new MusicPlayerState(list,
            ...
            ConcertoCommon.getPlatform().getLogger().warn("Error parsing JSON from music.json: {}", e.toString());
            return new MusicPlayerState();
        }
    }

    public static String toRaw(MusicPlayerState status) {
        JsonArray array = new JsonArray();
