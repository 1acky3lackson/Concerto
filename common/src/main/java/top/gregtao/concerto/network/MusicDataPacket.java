...
public class MusicDataPacket {
    ...
    public CommonConcertoPayload toPacket(String senderName) {
        if (this.isS2C) throw new RuntimeException("Only for C2S packet");
        JsonObject object = MusicJsonParsers.to(this.music);
        CommonConcertoPayload payload = new CommonConcertoPayload(CommonConcertoPayload.Channel.MUSIC_DATA, "");
        if (object != null) {
    ...
    public CommonConcertoPayload toPacket() {
        CommonConcertoPayload payload = new CommonConcertoPayload(CommonConcertoPayload.Channel.MUSIC_DATA, "");
        JsonObject object = MusicJsonParsers.to(this.music);
        if (object != null) {
            payload.string = object + "\n" + (this.isS2C ? this.from : this.to);
    ...
    public static MusicDataPacket fromPacket(CommonConcertoPayload buf, boolean isS2C) throws UnsafeMusicException {
        try {
