...
public class NeteaseCloudPlaylist extends Playlist {
    ...
    public NeteaseCloudPlaylist(JsonObject object, boolean isAlbum, boolean simply) {
        super(isAlbum ? NeteaseCloudApiClient.INSTANCE.parseAlbumJson(object, NeteaseCloudMusic.Level.byRank(ConcertoCommon.getPlatform().getNeteaseMusicQuality()), simply) :
                NeteaseCloudApiClient.INSTANCE.parsePlaylistJson(object, NeteaseCloudMusic.Level.byRank(ConcertoCommon.getPlatform().getNeteaseMusicQuality()), simply), isAlbum);
        this.simply = simply;
    ...
    Pair<ArrayList<Music>, PlaylistMetaData> loadData() {
        return this.isAlbum() ? NeteaseCloudApiClient.INSTANCE.getAlbum(this.id, NeteaseCloudMusic.Level.byRank(ConcertoCommon.getPlatform().getNeteaseMusicQuality())) :
                NeteaseCloudApiClient.INSTANCE.getPlaylist(this.id, NeteaseCloudMusic.Level.byRank(ConcertoCommon.getPlatform().getNeteaseMusicQuality()));
    }
