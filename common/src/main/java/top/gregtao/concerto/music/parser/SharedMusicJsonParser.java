...
public class SharedMusicJsonParser implements JsonParser<SharedMusic> {
    ...
    public SharedMusic fromJson(JsonObject object) {
        return new SharedMusic(object.get("path").getAsString(), CommonTextUtil.fromBase64(object.get("lyrics").getAsString()),
                CommonTextUtil.fromBase64(object.get("sub_lyrics").getAsString()), object.get("start_time").getAsLong(),
                object.get("start_byte").getAsLong());
    }

    @Override
    public JsonObject toJson(JsonObject object, SharedMusic music) {
        object.addProperty("lyrics", CommonTextUtil.toBase64(music.getRawLyrics()));
        object.addProperty("sub_lyrics", CommonTextUtil.toBase64(music.getRawSubLyrics()));
