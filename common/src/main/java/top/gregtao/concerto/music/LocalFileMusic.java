...
public class LocalFileMusic extends PathFileMusic {
    ...
    public LocalFileMusic(String rawPath) throws UnsafeMusicException {
        super(new File(CommonTextUtil.trimSurrounding(rawPath, "\"", "\"")).getAbsolutePath());
        String suffix = HttpUtil.getSuffix(this.getRawPath()).substring(1).toLowerCase();
        if (!FORMATS.contains(suffix)) {
            ConcertoCommon.getPlatform().getLogger().warn("Unsupported source: {}", suffix);
    ...
    public Pair<Lyrics, Lyrics> getLyrics() throws IOException {
        ...
        try {
            ...
            try {
                ...
                ConcertoCommon.getPlatform().getLogger().warn("Error occurs while loading file: '{}'", this.getRawPath());
            }
        }
    ...
    public void load() {
        ...
        if (duration <= 0) {
            this.setMusicMeta(new TimelessMusicMetaData(
                    author == null || author.isEmpty() ? CommonTextUtil.getTranslatable("concerto.unknown") : author,
