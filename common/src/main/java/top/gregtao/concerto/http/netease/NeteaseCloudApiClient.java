...
public class NeteaseCloudApiClient extends HttpApiClient {
    ...
    public static Supplier<NeteaseCloudMusic.Level> QUALITY_SUPPLIER = () -> NeteaseCloudMusic.Level.HIRES;

    // Abstract MAX_SIZE or remove dependency on MusicPlayerHandler
    public static int MAX_PLAYLIST_SIZE = 1000;
    ...
    public Pair<ArrayList<Music>, PlaylistMetaData> getPlaylist(String id, NeteaseCloudMusic.Level level) {
        try {
            String url = "http://music.163.com/api/v6/playlist/detail?id=" + id + "&n=" + MAX_PLAYLIST_SIZE;
            ...
            ConcertoCommon.getPlatform().getLogger().warn("Error while getting playlist {}: {}", id, e.getMessage());
    ...
    public Pair<ArrayList<Music>, PlaylistMetaData> getAlbum(String id, NeteaseCloudMusic.Level level) {
        try {
            ...
            ConcertoCommon.getPlatform().getLogger().warn("Error while getting album {}: {}", id, e.getMessage());
            return Pair.of(new ArrayList<>(), PlaylistMetaData.EMPTY);
        }
    }
    ...
    public List<Music> searchMusic(String keyword, int page) {
        try {
            ...
            array.forEach(element -> musics.add(new NeteaseCloudMusic(element.getAsJsonObject(), QUALITY_SUPPLIER.get())));
            return musics;
        } catch (Exception e) {
            ConcertoCommon.getPlatform().getLogger().warn("Error while searching for music '{}': {}", keyword, e.getMessage());
    ...
    public List<NeteaseCloudPlaylist> searchPlaylist(String keyword, int page) {
        try {
            ...
            ConcertoCommon.getPlatform().getLogger().warn("Error while searching for playlist '{}': {}", keyword, e.getMessage());
            return new ArrayList<>();
        }
    }
    ...
    public List<NeteaseCloudPlaylist> searchAlbum(String keyword, int page) {
        try {
            ...
            ConcertoCommon.getPlatform().getLogger().warn("Error while searching for album '{}': {}", keyword, e.getMessage());
            return new ArrayList<>();
        }
    }
    ...
    public FixedPlaylist getDailyRecommendation() {
         ...
         songs.forEach(element -> musics.add(new NeteaseCloudMusic(element.getAsJsonObject(), QUALITY_SUPPLIER.get())));
         return new FixedPlaylist(musics, new PlaylistMetaData(ConcertoCommon.getPlatform().translate("concerto.source.netease_cloud"),
                 ConcertoCommon.getPlatform().translate("concerto.screen.daily_recommendation"), "", ""), false);
    }
