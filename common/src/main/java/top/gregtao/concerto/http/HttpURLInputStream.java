...
public class HttpURLInputStream extends InputStream {
    ...
    public HttpURLInputStream(URL url, int startBytePos, Supplier<String> urlSupplier) throws IOException {
        ...
        if (this.connection.getResponseCode() == 200) {
            ...
            ConcertoCommon.getPlatform().getLogger().info("Connected to {}", url);
        } else {
            String message = this.connection.getResponseCode() + " - couldn't access to: " + url;
            ConcertoCommon.getPlatform().getLogger().error(message);
    ...
    public static int getTotalBytes(URL url) {
        try(HttpURLInputStream stream = new HttpURLInputStream(url)) {
            ...
            ConcertoCommon.getPlatform().getLogger().error(e.getMessage());
            return 0;
        }
    }
    ...
    private void connect() throws IOException {
        ...
        if (this.connection.getResponseCode() == 206 || this.connection.getResponseCode() == 200) {
            ...
            if (this.connection.getResponseCode() == 403 && this.urlSupplier != null) {
                ...
                ConcertoCommon.getPlatform().getLogger().warn("Trying to request for a new url.");
            }
            throw new IOException(message);
        }
    }
    ...
    private void reconnect() throws IOException {
        ConcertoCommon.getPlatform().getLogger().warn("Connection Reset: Trying reconnecting to {}", this.url);
        try {
            this.disconnect();
            this.connect();
        } catch (IOException e) {
            ConcertoCommon.getPlatform().getLogger().error("Failed to reconnect!");
            if (++this.retryCount > 10) {
                ConcertoCommon.getPlatform().getLogger().error("Failed to reconnect for 10 times! Closing...");
                this.close();
            }
        }
    }
